...
namespace avs
{
	...
	template <typename U, typename T> bool operator==(const std::map<U, T> &m1, const std::map<U, T> &m2)
	{
		if (m1.size() != m2.size())
		{
			return false;
	...
	template <typename T> bool operator==(const std::vector<T> &m1, const std::vector<T> &m2)
	{
		if (m1.size() != m2.size())
		{
			return false;
	...
	struct Mesh
	{
		...
		template <typename OutStream> friend OutStream &operator<<(OutStream &out, const Mesh &mesh)
		{
		...
		template <typename InStream> friend InStream &operator>>(InStream &in, Mesh &mesh)
		{
			in >> mesh.name;
	...
	*/
	...
	public:
		...
		virtual avs::Result CreateAnimation(avs::uid server_uid, avs::uid id, teleport::core::Animation &animation, avs::AxesStandard sourceAxesStandard) = 0;
	...
	public:
		...
		virtual Result decode(avs::uid						  server_uid,
							  const void					 *buffer,
							  size_t						  bufferSizeInBytes,
							  GeometryPayloadType			  type,
							  GeometryTargetBackendInterface *target,
							  avs::uid						  uid)		   = 0;
	};
} // avs
#ifdef _MSC_VER
