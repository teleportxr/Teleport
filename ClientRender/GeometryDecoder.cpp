...
avs::Result GeometryDecoder::DecodeGltf(const GeometryDecodeData &geometryDecodeData)
{
	...
	if (scene.get())
	{
		...
		CreateSubScene(subSceneDG,
					   ...
					   geometryDecodeData.uid,
					   geometryDecodeData.sourceAxesStandard);
		return DecodeDracoScene(subSceneDG,
								geometryDecodeData.target,
								geometryDecodeData.filename_or_url,
								geometryDecodeData.server_or_cache_uid,
								geometryDecodeData.uid,
...
avs::Result GeometryDecoder::DecodeDracoScene(core::DecodedGeometry				   &subSceneDG,
											  clientrender::ResourceCreator		   *target,
											  std::string							filename_url,
...
avs::Result GeometryDecoder::CreateFromDecodedGeometry(clientrender::ResourceCreator *target, core::DecodedGeometry &dg, const std::string &name)
{
	...
	for (auto m : dg.nodes)
	{
		...
		platform::crossplatform::AxesStandard targetAxesStandard=platform::crossplatform::AxesStandard::Engineering;
		avs::Transform &T=const_cast<avs::Transform &>(avsNode.localTransform);
		T.position;
		if (dg.axesStandard != targetAxesStandard)
		{
			vec3 &pos = const_cast<vec3&>(avsNode.localTransform.position);
			vec4 &rot = const_cast<vec4&>(avsNode.localTransform.rotation);
			vec3 &sca = const_cast<vec3&>(avsNode.localTransform.scale);
			pos = platform::crossplatform::ConvertPosition(
				dg.axesStandard, targetAxesStandard, avsNode.localTransform.position);
			platform::crossplatform::Quaternionf q = platform::crossplatform::ConvertRotation(
				dg.axesStandard, targetAxesStandard, avsNode.localTransform.rotation);
			rot = (const float *)&q;
			sca	= platform::crossplatform::ConvertScale(
				   dg.axesStandard, targetAxesStandard, avsNode.localTransform.scale);
		}
		if(dg.axesStandard!=targetAxesStandard)
		{
			for(auto &b:avsNode.inverseBindMatrices)
			{
				mat4 &B=const_cast<mat4&>(b);
				//B.transpose();
				B=platform::crossplatform::ConvertMatrix(dg.axesStandard,targetAxesStandard,B);
				//B.transpose();
			}
		}
...
avs::Result GeometryDecoder::decodeTexturePointer(GeometryDecodeData &geometryDecodeData)
{
	...
	// Leading byte of every pointer body: the axes standard. A placeholder for texture
	// interpretation here — textures have no geometric frame.
	[[maybe_unused]] platform::crossplatform::AxesStandard axesStandard =
...
avs::Result GeometryDecoder::decodeAnimation(GeometryDecodeData &geometryDecodeData)
{
	...
	if (geometryDecodeData.geometryFileFormat == GeometryFileFormat::GLTF_BINARY)
	{
		...
		return geometryDecodeData.target->CreateAnimation(
			geometryDecodeData.server_or_cache_uid, animationID, animation, (avs::AxesStandard)geometryDecodeData.sourceAxesStandard);
	}
	...
	geometryDecodeData.target->CreateAnimation(
		geometryDecodeData.server_or_cache_uid, animationID, animation, (avs::AxesStandard)geometryDecodeData.sourceAxesStandard);
