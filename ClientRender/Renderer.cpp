...
Renderer::~Renderer()
{
	// Cancel any sign-in still waiting on the browser and join the identity worker here, rather
	// than leaving it to the global's destructor at static-destruction time.
	client::identity.Shutdown();
...
void Renderer::InitLocalGeometry()
{
	...
	if (renderState.commonFontAtlas)
	{
		...
		geometryDecoder.decodeFromFile(
			0, renderState.commonFontAtlas->font_texture_path, avs::GeometryPayloadType::Texture, &localResourceCreator, font_texture_uid);
	}
	geometryDecoder.WaitFromDecodeThread();
	...
	geometryDecoder.decodeFromFile(0,
								   "assets/localGeometryCache/textures/specularRenderTexture_neon.ktx2",
								   avs::GeometryPayloadType::Texture,
								   &localResourceCreator,
