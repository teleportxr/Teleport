...
ResourceCreator::ResourceCreator() : textureTranscodeThread(&ResourceCreator::thread_TranscodeTextures, this)
{
}
...
void ResourceCreator::Initialize(platform::crossplatform::RenderPlatform *r, clientrender::VertexBufferLayout::PackingStyle packingStyle)
{
	...
	assert(packingStyle == clientrender::VertexBufferLayout::PackingStyle::GROUPED ||
		   packingStyle == clientrender::VertexBufferLayout::PackingStyle::INTERLEAVED);
...
template <typename indexType> bool GenerateTangents(avs::MeshElementCreate &meshElementCreate, std::vector<vec4> &tangent)
{
...
clientrender::Texture::Format textureFormatFromAVSTextureFormat(avs::TextureFormat format)
{
	switch (format)
	{
	case avs::TextureFormat::INVALID:
		return clientrender::Texture::Format::FORMAT_UNKNOWN;
	case avs::TextureFormat::G8:
		return clientrender::Texture::Format::R8;
	case avs::TextureFormat::BGRA8:
		return clientrender::Texture::Format::BGRA8;
	case avs::TextureFormat::BGRE8:
		return clientrender::Texture::Format::BGRA8;
	case avs::TextureFormat::RGBA16:
		return clientrender::Texture::Format::RGBA16;
	case avs::TextureFormat::RGBE8:
		return clientrender::Texture::Format::RGBA8;
	case avs::TextureFormat::RGBA16F:
		return clientrender::Texture::Format::RGBA16F;
	case avs::TextureFormat::RGBA32F:
		return clientrender::Texture::Format::RGBA32F;
	case avs::TextureFormat::RGBA8:
		return clientrender::Texture::Format::RGBA8;
	case avs::TextureFormat::D16F:
		return clientrender::Texture::Format::DEPTH_COMPONENT16;
	case avs::TextureFormat::D24F:
		return clientrender::Texture::Format::DEPTH_COMPONENT24;
	case avs::TextureFormat::D32F:
		return clientrender::Texture::Format::DEPTH_COMPONENT32F;
	case avs::TextureFormat::MAX:
		return clientrender::Texture::Format::FORMAT_UNKNOWN;
	default:
		exit(1);
	}
...
void ResourceCreator::CreateTexture(avs::uid server_uid, avs::uid id, const avs::Texture &texture)
{
	...
	TELEPORT_LOG("Received texture {0} ({1}), awaiting decompression.", id, texture.name);
	{
		...
		texturesToTranscode.emplace_back(
			std::make_shared<UntranscodedTexture>(server_uid, id, texture.compressedData, texInfo, texture.name, texture.compression, texture.valueScale));
	}
}
...
void ResourceCreator::CreateMaterial(avs::uid server_uid, avs::uid id, const avs::Material &material)
{
	...
	geometryCache->AddTextureToMaterial(material.pbrMetallicRoughness.metallicRoughnessTexture,
										vec4{material.pbrMetallicRoughness.roughnessMultiplier,
	...
	else
	{
		...
		TELEPORT_INTERNAL_COUT("CreateMaterial {0} ({1}) as incomplete: missing textures: {2} awaiting {3} resources.",
							   ...
							   material.name,
							   texlist,
							   RESOURCES_AWAITED(incompleteMaterial));
	}
}
...
avs::Result ResourceCreator::CreateAnimation(avs::uid server_uid, avs::uid id, teleport::core::Animation &animation, avs::AxesStandard sourceAxesStandard)
{
	std::shared_ptr<GeometryCache> geometryCache = GeometryCache::GetGeometryCache(server_uid);
	geometryCache->ReceivedResource(id);
	...
	else
	{
		...
		std::shared_ptr<clientrender::Animation> completeAnimation =
			std::make_shared<clientrender::Animation>(animation.name, animation.duration, boneKeyframeLists);
		geometryCache->CompleteAnimation(id, completeAnimation);
	}
	return avs::Result::OK;
}
...
static clientrender::Texture::Format VkFormatToTeleportFormat(VkFormat f)
{
	...
	case VK_FORMAT_R8G8B8A8_SNORM:
		...
		// Compressed Format 0xSnn1..0xSnnF   (Keys 0x00Bv..0x00Bv) S =1 is signed, 0 = unsigned, B =Block Compressors 1..7 (BC1..BC7) and v > 1 is a variant
		// like signed or swizzle
	case VK_FORMAT_BC2_UNORM_BLOCK:
		return clientrender::Texture::Format::RGBA8; // compressed texture format with explicit alpha for Microsoft DirectX10. Identical to DXT3. Eight bits per
													 // pixel.
	case VK_FORMAT_BC3_UNORM_BLOCK:
		return clientrender::Texture::Format::RGBA8; // compressed texture format with interpolated alpha for Microsoft DirectX10. Identical to DXT5. Eight bits
													 // per pixel.
	case VK_FORMAT_BC4_UNORM_BLOCK:
		return clientrender::Texture::Format::RGBA8; // compressed texture format for Microsoft DirectX10. Identical to ATI1N. Four bits per pixel.
...
static teleport::clientrender::Texture::CompressionFormat VkFormatToCompressionFormat(VkFormat f)
{
	...
	case VK_FORMAT_BC2_UNORM_BLOCK:
		return teleport::clientrender::Texture::CompressionFormat::BC3; // compressed texture format with explicit alpha for Microsoft DirectX10. Identical to
																		// DXT3. Eight bits per pixel.
	case VK_FORMAT_BC3_UNORM_BLOCK:
		return teleport::clientrender::Texture::CompressionFormat::BC3; // compressed texture format with interpolated alpha for Microsoft DirectX10. Identical
																		// to DXT5. Eight bits per pixel.
	case VK_FORMAT_BC4_UNORM_BLOCK:
		return teleport::clientrender::Texture::CompressionFormat::BC4; // compressed texture format for Microsoft DirectX10. Identical to ATI1N. Four bits per
																		// pixel.
	case VK_FORMAT_BC4_SNORM_BLOCK:
		return teleport::clientrender::Texture::CompressionFormat::BC4; // compressed texture format for Microsoft DirectX10. Identical to ATI1N. Four bits per
																		// pixel.
...

		{
			...
			if (transcoding->compressionFormat == avs::TextureCompression::MULTIPLE_PNG)
			{
				...
				transcoding->textureCI->type =
					transcoding->textureCI->depth > 1 ? clientrender::Texture::Type::TEXTURE_3D : clientrender::Texture::Type::TEXTURE_2D;
				if (cubemap)
				{
				...
				for (int i = 0; i < num_images; i++)
				{
					...
					unsigned char *target =
						teleport::stbi_load_from_memory(basePtr + imageOffsets[i], (int)imageSizes[i], &mipWidth, &mipHeight, &num_channels, (int)4);
					 ...
					 transcoding->compressionFormat == avs::TextureCompression::UNCOMPRESSED)
			{
				...
				for (int i = 0; i < 1; i++)
				{
					...
					int			   mipWidth = 0, mipHeight = 0;
					unsigned char *stb =
						teleport::stbi_load_from_memory(transcoding->data.data(), transcoding->data.size(), &mipWidth, &mipHeight, &num_channels, (int)4);
