...
void InstanceRenderer::RenderInstancedMeshes(crossplatform::GraphicsDeviceContext &deviceContext, const InstancedRender &instancedRender)
{
	...
	// Fill the instance buffer if the instance set changed, or if any instance has moved
	// since we last uploaded. The model pointers reference NodeState::renderModelMatrix,
	// which is recomputed every frame — a server-driven node (an avatar following its
	// client, say) moves without the instance set changing at all, and only this check
	// gets the new transform to the GPU.
	bool refillInstanceBuffer = !instancedRender.valid;
	if (!refillInstanceBuffer && instancedRender.uploadedModels.size() == instancedRender.meshInstanceRenders.size())
	{
		size_t i = 0;
		for (auto &m : instancedRender.meshInstanceRenders)
		{
			if (memcmp(&instancedRender.uploadedModels[i], m.second.model, sizeof(mat4)) != 0)
			{
				refillInstanceBuffer = true;
				break;
			}
			i++;
		}
	...
	if (refillInstanceBuffer)
	{
		...
		instancedRender.uploadedModels.resize(instancedRender.meshInstanceRenders.size());
		int i = 0;
		for (auto &m : instancedRender.meshInstanceRenders)
		{
			...
			instancedRender.uploadedModels[i] = inst[i].model;
			i++;
		}
