...
namespace teleport
{
	namespace clientrender
	{
		...
		{
			AVSTextureImpl(platform::crossplatform::Texture *t) : texture(t)
			{
			}
		...
		struct RenderState
		{
			...
			platform::crossplatform::ConstantBuffer<StereoCameraConstants, platform::crossplatform::ResourceUsageFrequency::FEW_PER_FRAME>
				stereoCameraConstants;
			platform::crossplatform::ConstantBuffer<CubemapConstants> cubemapConstants;
			platform::crossplatform::ConstantBuffer<TeleportSceneConstants, platform::crossplatform::ResourceUsageFrequency::FEW_PER_FRAME>
				teleportSceneConstants;
		...
		public:
			...
			// So, the mapping of a single element of one mesh in a node is:
			// pass -> PassRender, material_uid -> materialRender, NodeElementHash(root_id, node->id, cache_uid, element) -> meshRender
			//												 or, InstanceHash(cache_uid, mesh.get(), node.get(), element) -> instancedRender
			//
			...
			struct ElementState
			{
				std::weak_ptr<MaterialRender> materialRender;
				uint64_t hash=0;
			};
			//! Per-frame node state, either per-node or per-rendered instance of a node.
			struct NodeState
			{
				mat4 renderModelMatrix;			//!< the model matrix in-use, including all transforms (e.g. subCache transforms, etc).
				std::vector<ElementState> elementStates;
			};
			//! A container for the specific state of a node instance.
			//! So we can render SubScenes, we have to have somewhere to put the state that belongs
			//! to the parent cache's instance of the child cache's nodes.
			struct SubSceneNodeStates
			{
				avs::uid root_id = 0;					  //!< The root node containing the subscene.
				std::map<avs::uid, NodeState> nodeStates; //!< The node states for this cache.
			};
			//! Map from the main scene node that mounts a subScene to the states for that instance.
			std::map<avs::uid, SubSceneNodeStates> subSceneStatesMap;
			struct NodeRender
			{
		...
		public:
			...
			void RenderVideoTexture(platform::crossplatform::GraphicsDeviceContext &deviceContext,
									...
									platform::crossplatform::Texture *targetTexture,
									const char *technique,
									const char *shaderTexture);
			void RecomposeVideoTexture(platform::crossplatform::GraphicsDeviceContext &deviceContext,
									   platform::crossplatform::Texture *srcTexture,
									   platform::crossplatform::Texture *targetTexture,
									   const char *technique);
			void RecomposeCubemap(platform::crossplatform::GraphicsDeviceContext &deviceContext,
								  platform::crossplatform::Texture *srcTexture,
								  platform::crossplatform::Texture *targetTexture,
								  int mips,
								  int2 sourceOffset);
			...
			void UpdateGeometryCacheForRendering(platform::crossplatform::GraphicsDeviceContext &deviceContext,
												 SubSceneNodeStates &subSceneNodeStates,
												 std::shared_ptr<clientrender::GeometryCache> geometryCache);
			void UpdateNodeForRendering(platform::crossplatform::GraphicsDeviceContext &deviceContext,
			...
			void AddNodeToInstanceRender(avs::uid cache_uid, SubSceneNodeStates &subSceneNodeStates, avs::uid node_uid);
			void RemoveGeometryCacheFromInstanceRender(avs::uid cache_uid,SubSceneNodeStates &subSceneNodeStates);
			void RemoveNodeFromInstanceRender(avs::uid cache_uid, SubSceneNodeStates &subSceneNodeStates, avs::uid node_uid);
			void UpdateNodeInInstanceRender(avs::uid cache_uid, avs::uid root_id, avs::uid node_uid);
			void AddNodeMeshToInstanceRender(avs::uid cache_uid,
											 SubSceneNodeStates &subSceneNodeStates,
