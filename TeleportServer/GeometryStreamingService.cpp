...
void GeometryStreamingService::confirmResource(avs::uid resource_uid)
{
	...
	if (auto node = geometryStore->getNode(resource_uid))
	{
		...
		{
			TELEPORT_WARN(
				"GeometryStreamingService::confirmResource Trying to decrement Node {0} priority {1} but it's not in the list.", resource_uid, node->priority);
			// This can happen if the node is in nodesToStream, but was not in streamedNodes. But the client already had it, or received it before it was
			// removed.
		}
		else
		{
...
#if TELEPORT_DEBUG_RESOURCE_STREAMING
			TELEPORT_COUT << "GeometryStreamingService::confirmResource Node " << resource_uid << " priority " << node->priority << ", count "
...
void GeometryStreamingService::GetMeshNodeResources(avs::uid nodeID, const avs::Node &node, std::vector<avs::MeshNodeResources> &outMeshResources) const
{
	...
	for (avs::uid material_uid : node.materials)
	{
		...
		material.texture_uids = {thisMaterial->pbrMetallicRoughness.baseColorTexture.index,
