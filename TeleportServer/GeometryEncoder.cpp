...
avs::Result GeometryEncoder::encodeMeshes(std::vector<avs::uid> missingUIDs)
{
	...
	for (avs::uid uid : missingUIDs)
	{
		...
		if (compressedMesh && compressedMesh->meshCompressionType != avs::MeshCompressionType::NONE)
		{
			...
			uint16_t nameLength = (uint16_t)compressedMesh->name.length();
			put(nameLength);
...
avs::Result GeometryEncoder::encodeNodes(std::vector<avs::uid> nodeUids)
{
	...
	for (const avs::uid &uid : nodeUids)
	{
		...
		// Push name length.
		uint16_t nameLength = (uint16_t)node->name.length();
		put(nameLength);
		// Push name.
		...
		else
		{
			...
			{
				uint16_t urlLength = (uint16_t)node->url.length();
				put(urlLength);
				put((uint8_t *)node->url.data(), urlLength);
				uint16_t queryLength = (uint16_t)node->query_url.length();
				put(queryLength);
...
avs::Result GeometryEncoder::encodeSkeleton(avs::uid skeletonID)
{
	...
	if (skeleton)
	{
		...
		// Push name length.
		uint16_t nameLength = (uint16_t)skeleton->name.length();
		put(nameLength);
		// Push name.
...
avs::Result GeometryEncoder::encodeAnimation(avs::uid animationID)
{
	...
	if (animation)
	{
		...
		// Push name length.
		uint16_t nameLength = (uint16_t)animation->name.length();
		put(nameLength);
		// Push name.
...
avs::Result GeometryEncoder::encodeMaterials(std::vector<avs::uid> missingUIDs)
{
	...
	for (avs::uid uid : missingUIDs)
	{
		...
		if (material)
		{
			...
			uint16_t nameLength = (uint16_t)material->name.length();
...
avs::Result GeometryEncoder::encodeTexturesBackend(std::vector<avs::uid> missingUIDs, bool)
{
	...
	for (avs::uid uid : missingUIDs)
	{
		...
		if (texture)
		{
			...
			uint16_t nameLength = (uint16_t)texture->name.length();
